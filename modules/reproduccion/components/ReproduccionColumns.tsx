// modules/reproduccion/components/reproduccionColumns.tsx
'use client';
...
interface GetReproduccionColumnsProps {
  ...
  onDelete?: (item: Reproduccion) => void; // <-- Cambiado de (id: string) a (item: Reproduccion)
  permisos?: {
...
export function getReproduccionColumns({
  onEdit,
  onDelete,
  permisos = { puede_editar: true, puede_eliminar: true }
}: GetReproduccionColumnsProps): Column<Reproduccion>[] {
  return [
    { 
          ...
          <div className="text-xs space-y-0.5">
            {item.fecha_parto ? (
              item.fecha_secado ? (
                ...
                <span className="text-emerald-600 font-semibold block">En Produccion: {diasLactancia} días</span>
              )
            ) : (
      ...
      render: (_, item) => (
        <ActionDropdown actions={[
          ...
          { 
            ...
            onClick: () => onDelete?.(item), // <-- Cambiado de item.id! a item completo
            danger: true,
